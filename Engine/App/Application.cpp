...
void Application::initVk() {
    ...
    _instance = std::make_shared<Instance>(std::string("vulkanApp"), instanceExtensions, validationLayers);
    auto physicalDevice = createPhysicalDevice();
    auto surface = _window->createSurface(_instance->getHandle());
    ...
    createAllocator();
    _context = std::make_shared<RenderContext>(_device, surface, _window);
    createCommandBuffer();
    createRenderPass();
    createDepthStencil();
    createFrameBuffers();
...
void Application::draw(CommandBuffer &commandBuffer, RenderTarget &target) {
    ...
    drawRenderPasses(commandBuffer,target);

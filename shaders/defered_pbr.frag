...
vec3 SampleBicubic1(uint texture_idx,in vec2 uv,float u) {

    ivec2 size = textureSize(scene_textures[texture_idx],0);
    ...
    pos_coord /= vec2(size);
    vec3 sampled_val = pos_weights_sum * texture(scene_textures[texture_idx], pos_coord).xyz;
    ...
    if (neg_weights_sum != 0.0f) {
        ...
        neg_coord /= vec2(size);
        sampled_val += texture(scene_textures[texture_idx],neg_coord).xyz * (-neg_weights_sum);
    }
    return sampled_val;
}
...
vec2 stochastic(in vec2 uv, in ivec2 size, in bool need_stochastic,inout uvec4 seed) {
    if (need_stochastic) {
        return stochastic_bilin(uv, rand2(seed),size);
    }
    else {
        return uv;
    }
...
vec3 stochastic_sample(uint texture_idx,in vec2 uv,inout uvec4 seed,bool need_stochastic) {
    float aniso = computeLodAniso(uv, 0.0, 13, rand1(seed));

    vec2 sample_uv = stochastic(uv, textureSize(scene_textures[texture_idx], 0),need_stochastic,seed);
    return texture(scene_textures[texture_idx], sample_uv,aniso).xyz;
...
vec3 getNormal(const int texture_idx, vec2 uv,inout uvec4 seed,bool need_stochastic)
{
  ...
  //  vec2 lodInfo = textureQueryLod(scene_textures[texture_idx], uv);
    ...
    vec3 tangentNormal;
     if(!need_stochastic) tangentNormal = texture(scene_textures[texture_idx], uv).xyz * 2.0 - 1.0;
  //  else tangentNormal =SampleBicubic1(texture_idx,uv,rand1(seed)) *2 - 1.0;
    else tangentNormal = stochastic_sample(texture_idx,uv,seed,need_stochastic) * 2.0 - 1.0;
...
void main(void)
{
    ...
    if (material.pbrMetallicRoughnessTexture > -1)
    {
        ...
        vec4 mrSample = texture(scene_textures[material.pbrMetallicRoughnessTexture], sample_uv);
    ...
    if (material.pbrBaseColorTexture > -1)
    {
        ...
        vec2 sample_uv = stochastic(uv, size, need_stochastic,seed);
        baseColor *= texture(scene_textures[material.pbrBaseColorTexture], sample_uv);

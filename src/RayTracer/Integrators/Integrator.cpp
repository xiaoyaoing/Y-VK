...
void Integrator::initScene(Scene& scene)
{
    ...
    // vertexBuffer = std::make_unique<Buffer>(device, positionBufferSize, staging_flags, VMA_MEMORY_USAGE_CPU_TO_GPU);
    // indexBuffer = std::make_unique<Buffer>(device, positionBufferSize, staging_flags, VMA_MEMORY_USAGE_CPU_TO_GPU);

    
   // uint32_t 
    auto commandBuffer = device.createCommandBuffer(VK_COMMAND_BUFFER_LEVEL_PRIMARY, true);
    auto copyBuffer = [commandBuffer](const Primitive & primitive,Buffer & dstBuffer,const std::string & attributeName) {
...
void Integrator::buildBLAS()
{
    ...
    for(uint32 i = 0 ; i < primitives.size() ; i++)
    {
        ...
        accelerationStructureGeometry.geometry.triangles.vertexData.deviceAddress = vertexBuffer->getDeviceAddress() + primitive.vertexOffset * sizeof(glm::vec3);
        ...
        accelerationStructureGeometry.geometry.triangles.indexData.deviceAddress = indexBuffer->getDeviceAddress() + primitive.indexOffset * sizeof(uint32_t);

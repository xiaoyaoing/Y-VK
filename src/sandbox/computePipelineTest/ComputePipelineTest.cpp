...
void ComputePipelineTest::prepare() {
    ...
    camera->setPerspective(60.0f, static_cast<float>(mWidth) / static_cast<float>(mHeight), 512.0f, 0.1f);
...
void ComputePipelineTest::drawFrame(RenderGraph& rg) {
    ...
    rg.addComputePass(
        "Compute Pass", [&](auto& builder, auto& settings) {
        
        auto bufferHandle = rg.importBuffer("storageBuffer",storageBuffer.get());
        builder.writeBuffer(bufferHandle);
        rg.getBlackBoard().put("storageBuffer",bufferHandle); }, [&](auto& context) {
    ...
    rg.addGraphicPass(
        ...
        [&](auto& context) {
            auto& commandBuffer = context.commandBuffer;
            renderContext->getPipelineState().setPipelineLayout(*graphics.pipelineLayout).setInputAssemblyState({.topology = VK_PRIMITIVE_TOPOLOGY_POINT_LIST}).setVertexInputState({.bindings = {vkCommon::initializers::vertexInputBindingDescription(0, sizeof(Particle), VK_VERTEX_INPUT_RATE_VERTEX)}, .attributes = {vkCommon::initializers::vertexInputAttributeDescription(0, 0, VK_FORMAT_R32G32B32A32_SFLOAT, offsetof(Particle, pos)), vkCommon::initializers::vertexInputAttributeDescription(0, 1, VK_FORMAT_R32G32B32A32_SFLOAT, offsetof(Particle, vel))}}).setDepthStencilState({.depthTestEnable = false}).setRasterizationState({.cullMode = VK_CULL_MODE_NONE}).setColorBlendState({.attachments = {state}});

...
inline void readShaderResource(spirv_cross::CompilerReflection& compiler, VkShaderStageFlagBits stage, std::vector<ShaderResource>& resources, const spirv_cross::SmallVector<spirv_cross::Resource>& inputResources, ShaderResourceType type) {
    for (auto& spirvResource : inputResources) {
        ShaderResource resource{};
        //resource.type =
        resource.type   = type;
